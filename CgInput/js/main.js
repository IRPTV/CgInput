$(function () {
	...
	*/
	...
	/*!
	* Fixed page header when user scrolls down
	*/
	...
	*/
	...
	/*!
	* datepicker plugin for datepicker inputs
	* changing date will result in new items loading
	*/
	...
	// Add item button. Insert items on click
	...
	/*!
	* Parse date. process time string splitted by '/' and create an array
	*/
	...
	*/
	...
	if ($("#times").length || $("#dtimes").length)
		timeSelect($time); // select time of page open
	...
	*/
	...
	/*!
	* Delete all items in selected time, language, etc.
	*/
    $("#delete-all").click(function (e) {
        if (!$(this).hasClass('disabled')) {
            if (confirm('آیا مطمئن هستید همه موارد حذف شوند؟')) {
                // if (!$("#datepicker").val().length || $("#language").val() < 1 || !$crawl.val()) {
                if (!$("#datepicker").val().length || $("#language").find("option:selected").val() < 1 || !$crawl.val()) {
                    if (!$("#datepicker").val().length)
                        raiseError($("#datepicker"));
                    // if ($("#language").val() < 1)
                    if ($("#language").find("option:selected").val() < 1)
                        raiseError($("#language"));
                    if (!$crawl.val())
                        raiseError($crawl);
                } else {
    ...
    /*!
	* Get info for "info bar"
	*/	
	function getPublishInfo(Times_Id, Crawl_date) {
        var newDate = parseDate(Crawl_date);
        $.ajax({
            type: "POST",
            ...
            success: function (data) {
                ...
                if (data.d.LOCK) { // If time has passed and crawl is locked
                    $("#insert").hide();
    ...
    $dmodal.on('shown', function () {
        ...
        $("#slang").val($("#language").find("option:selected").val());
    ...
    $dmodal.delegate("#do-duplicate", 'click', function (event) {
        ...
        var params = {
			...
			, DestLang: $("#language").find("option:selected").val()
        }
    ...
    // }
	if ($("#language").find("option:selected").val() > 0) {
		loadCrawls($("#language").find("option:selected").val(), $crawl);
	}
    ...
    function checkUserAccessToLang() {
		$.ajax({
            type: "POST",
